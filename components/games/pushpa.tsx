    ...
    <div className="min-h-screen bg-slate-900 text-white relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 sm:p-6 border-b border-slate-700 gap-3 mb-4">
        <div className="flex items-center space-x-2 sm:space-x-4">
          <Button variant="ghost" size="sm" className="text-slate-300 hover:text-white" onClick={() => window.history.back()}>
            <ArrowLeft className="h-4 w-4 mr-1 sm:mr-2" />
            <span className="hidden sm:inline">Back to Games</span>
            <span className="sm:hidden">Back</span>
          </Button>
          <div>
            <h1 className="font-work-sans font-bold text-xl sm:text-2xl text-white">Pushpa</h1>
            <p className="text-slate-400 text-sm sm:text-base hidden sm:block">Truck-themed crash game with multiplier betting</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-2 sm:px-4 py-2 sm:py-4">
        <div className="w-full max-w-7xl bg-slate-800/20 rounded-lg p-2 sm:p-4 mb-3 sm:mb-6">
          ...
          <div className="relative overflow-hidden bg-gradient-to-b from-teal-900/40 via-cyan-800/30 to-emerald-700/40 rounded-lg mb-3 sm:mb-6" style={{ height: 'clamp(200px, 40vh, 320px)' }}>
            ...
            <div className="absolute top-2 sm:top-4 left-2 sm:left-6 right-2 sm:right-6">
              <div className="w-full h-1 sm:h-2 bg-gray-800/60 rounded-full overflow-hidden backdrop-blur-sm">
                <div
              ...
              <div className="text-xs sm:text-sm text-gray-300 mt-1 sm:mt-2 font-semibold">
                <span className="hidden sm:inline">Journey Progress: {Math.min(Math.floor((multiplier - 1) * 20), 80)}%</span>
                <span className="sm:hidden">{Math.min(Math.floor((multiplier - 1) * 20), 80)}%</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-4xl">
          <div className="bg-slate-800/50 rounded-lg p-3 sm:p-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
                  ...
                  <div className="flex space-x-1">
                    {quickBetAmounts.map((amount) => (
                      <Button
                        ...
                        size="sm"
                        variant="outline"
                        className="text-xs px-1 sm:px-2 py-1 bg-transparent"
              ...
              <div className="flex flex-row sm:flex-col space-x-2 sm:space-x-0 sm:space-y-2">
                <Button
                  size="sm"
                  variant="outline"
                  className="text-xs bg-transparent hover:bg-slate-600 flex-1 sm:flex-none"
            ...
            <div className="mt-4 sm:mt-6 text-center">
              {!gameActive ? (
                <Button
                  ...
                  className={`px-8 sm:px-16 py-3 sm:py-4 text-lg sm:text-xl font-bold rounded-lg transition-all duration-200 ${
                ...
                >
                  <span className="hidden sm:inline">CASH OUT ₹{(betAmount * multiplier).toFixed(2)}</span>
                  <span className="sm:hidden">CASH OUT ₹{Math.floor(betAmount * multiplier)}</span>
                </Button>
              )}
            </div>
          </div>
      ...
      {gameHistory.length > 0 && (
        <div className="fixed top-16 sm:top-20 right-2 sm:right-4 bg-slate-800/90 rounded-lg p-2 sm:p-4 max-w-xs">
