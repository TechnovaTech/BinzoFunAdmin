...
export function MinesGame() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 sm:p-6 border-b border-slate-700 gap-3">
      ...
      <div className="flex flex-col lg:flex-row min-h-[calc(100vh-120px)]">
        ...
        <div className="w-full lg:w-80 bg-slate-800 p-3 sm:p-6 space-y-4 sm:space-y-6 border-b lg:border-b-0 lg:border-r border-slate-700">
          <div className="flex bg-slate-700 rounded-lg p-1">
            <Button
              variant={gameMode === "manual" ? "default" : "ghost"}
              size="sm"
              className={`flex-1 text-xs sm:text-sm ${gameMode === "manual" ? "bg-slate-600 text-white" : "text-slate-300 hover:text-white"}`}
            ...
            <Button
              ...
              size="sm"
              className={`flex-1 text-xs sm:text-sm ${gameMode === "auto" ? "bg-slate-600 text-white" : "text-slate-300 hover:text-white"}`}
            ...
            <div className="flex items-center space-x-2 mb-2">
              <div className="flex-1 relative">
                <Input
                  ...
                  min="1"
                  max={balance}
                  className="bg-slate-700 border-slate-600 text-white pr-12 sm:pr-16 text-sm sm:text-base"
                  step="0.01"
                />
                <div className="absolute right-2 sm:right-3 top-1/2 transform -translate-y-1/2 text-orange-400 text-xs sm:text-sm">$</div>
              </div>
              <Button
                variant="outline"
                size="sm"
                className="bg-slate-700 border-slate-600 text-slate-300 hover:text-white px-2 sm:px-3 text-xs sm:text-sm"
            ...
            <Select value={mineCount.toString()} onValueChange={(value) => setMineCount(Number.parseInt(value))}>
              ...
              <SelectContent className="bg-slate-700 border-slate-600">
                {Array.from({ length: 20 }, (_, i) => i + 1).map((num) => (
                  <SelectItem key={num} value={num.toString()} className="text-white hover:bg-slate-600 text-sm">
                    {num}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          {gameState === "playing" && (
            <div className="space-y-4">
              ...
              >
                <span className="hidden sm:inline">Cash Out (${Math.floor(betAmount * currentMultiplier)})</span>
                <span className="sm:hidden">Cash Out ${Math.floor(betAmount * currentMultiplier)}</span>
              </Button>
              <Button
                onClick={resetGame}
                variant="outline"
                className="w-full border-slate-600 text-slate-300 hover:text-white bg-transparent text-sm sm:text-base"
              >
                New Game
              </Button>
            </div>
        ...
        <div className="flex-1 flex items-center justify-center p-3 sm:p-6 lg:p-8">
          <div className="w-full max-w-2xl">
            <div className="grid grid-cols-5 gap-2 sm:gap-3 mx-auto" style={{ maxWidth: "min(500px, 100vw - 2rem)" }}>
              {grid.map((tile) => (
                <Button
                  key={tile.index}
                  variant="outline"
                  ...
                  style={{ minHeight: "clamp(50px, 12vw, 80px)" }}
                >
                  {tile.revealed ? (
                    tile.isMine ? (
            ...
            {gameState === "betting" && (
              <div className="text-center mt-4 sm:mt-6 text-slate-400 text-sm sm:text-base px-2">
                <span className="hidden sm:inline">Click "Bet" to start the game and reveal tiles</span>
                <span className="sm:hidden">Tap "Bet" to start</span>
              </div>
            )}

            {gameState === "playing" && (
              <div className="text-center mt-4 sm:mt-6 text-slate-300 text-sm sm:text-base px-2">
                <span className="hidden sm:inline">Click tiles to reveal gems. Avoid the {mineCount} hidden mines!</span>
                <span className="sm:hidden">Tap tiles to reveal gems. Avoid {mineCount} mines!</span>
              </div>
            )}
          </div>
        </div>
