  ...
  const renderGrid = () => {
    ...
    for (let y = 0; y < GRID_HEIGHT; y++) {
      for (let x = 0; x < GRID_WIDTH; x++) {
        let cellContent = ""
        let cellClass = "w-8 h-8 sm:w-12 sm:h-12 lg:w-16 lg:h-16 flex items-center justify-center text-lg sm:text-2xl lg:text-4xl relative "
        ...
        if (playerPos.x === x && playerPos.y === y) {
          cellContent = "🐔"
          cellClass += "text-xl sm:text-3xl lg:text-5xl z-20 relative transform scale-x-[-1] "
        }
        ...
        obstacles.forEach((obstacle) => {
          ...
          if (obstacle.x === x && obstacleGridY === y) {
            ...
            switch (obstacle.type) {
              case "car":
                cellContent = "🚗"
                cellClass += "transform rotate-[270deg] text-lg sm:text-2xl lg:text-4xl drop-shadow-lg "
                break
              case "truck":
                cellContent = "🚛"
                cellClass += "transform rotate-[270deg] text-xl sm:text-3xl lg:text-5xl drop-shadow-lg "
                break
              case "log":
                cellContent = "🚐"
                cellClass += "transform rotate-[270deg] text-lg sm:text-2xl lg:text-4xl drop-shadow-lg "
        ...
        grid.push(
          ...
          >
            {cellContent}
            {x > 1 && x < 9 && y % 2 === 0 && (
              <div className="absolute inset-x-0 top-1/2 h-px sm:h-0.5 bg-yellow-400 opacity-60 transform -translate-y-1/2"></div>
            )}
          </div>,
        )
      }
      ...
      <div className="w-full max-w-4xl">
        <div className="flex flex-col sm:flex-row items-center justify-between mb-3 sm:mb-4 gap-2 sm:gap-0">
          ...
          <div className="text-center">
            <h1 className="font-bold text-lg sm:text-2xl text-white drop-shadow-lg">
              🐔 Chicken Road
            </h1>
          </div>
          <div className="text-white font-bold text-center sm:text-right">
            <div className="text-sm sm:text-base">Score: {score}</div>
            <div className="text-xs sm:text-sm text-yellow-400">Speed Round: {Math.floor(score / 500) + 1}</div>
          </div>
        </div>

        <div className="relative bg-black-200 rounded-lg p-1 sm:p-2 shadow-xl">
          ...
          <div className="absolute inset-0 pointer-events-none" >
            {[1.01, 1.03, 1.06, 1.10, 1.15, 1.19, 1.24, 1.30, 1.75].map((mult, i) => {
              ...
              return (
                <div 
                  key={i}
                  className="absolute w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 bg-gradient-to-br from-orange-400 to-orange-600 rounded-full border border-white sm:border-2 shadow-lg flex items-center justify-center text-white text-xs font-bold animate-pulse z-10"
                  style={{
                ...
                >
                  <span className="text-xs sm:text-xs lg:text-xs">{mult}x</span>
                </div>
              )
            })}
          </div>
          ...
          {gameOver && (
            <div className="absolute inset-0 bg-black/50 flex items-center justify-center rounded z-50">
              <div className="bg-white p-4 sm:p-6 rounded-lg text-center mx-4">
                <div className="text-lg sm:text-2xl font-bold text-red-500 mb-2">Game Over!</div>
                <div className="text-sm sm:text-base text-gray-700 mb-4">Final Score: {score}</div>
                <Button onClick={startGame} className="bg-green-600 hover:bg-green-700 w-full sm:w-auto">
                  Play Again
                </Button>
              </div>
            </div>
          ...
          {!isPlaying && !gameOver && (
            <div className="absolute inset-0 bg-black/50 flex items-center justify-center rounded z-50">
              <Button onClick={startGame} size="lg" className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 sm:py-4 sm:px-8 mx-4">
                🎮 Start Game
              </Button>
            </div>
          )}
        ...
        <div className="mt-3 sm:mt-4 text-center text-white">
          <p className="text-xs sm:text-sm opacity-80 px-2">
            <span className="hidden sm:inline">Use WASD or Arrow Keys to move • Avoid vehicles • Reach the right to score!</span>
            <span className="sm:hidden">Tap controls below • Avoid vehicles • Reach right to score!</span>
          </p>
        </div>
